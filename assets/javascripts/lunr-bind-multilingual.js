/* Prism.SourceGenerators.Docs — Lunr glue with per-locale indexes and CJK bigram queries. */
(function () {
  "use strict";
  ...
  function bigramCjk(text) {
    return text.replace(/[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uac00-\ud7af]+/g, function (run) {
  ...
  function localeUsesCjk(indexPath) {
    return indexPath.indexOf("/search/zh-cn/") >= 0 || indexPath.indexOf("/search/ja/") >= 0;
  }
  ...
  function tokenizeQuery(value, useCjk) {
    var normalized = useCjk && CJK_RE.test(value) ? bigramCjk(value) : value;
  ...
  function bind() {
    ...
    var useCjk = localeUsesCjk(indexMeta.content);
      ...
      if (!indexPromise) {
        indexPromise = fetch(indexMeta.content, { credentials: "same-origin" })
          ...
          .then(function (raw) {
            ...
            return { index: index, byRef: byRef };
          })
          .catch(function (err) {
            indexPromise = null;
            throw err;
    ...
    var runSearch = debounce(function (value) {
      ...
      loadIndex().then(function (state) {
        var terms = tokenizeQuery(trimmed, useCjk);
