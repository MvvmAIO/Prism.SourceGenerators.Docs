...
## PSG6001 — partial property 建议

| ID | 严重级别 | Title |
|----|----------|-------|
| **PSG6001** | Info | Use partial property for `[ObservableProperty]` (C# 13+) |

**代码修复：** 当启用 C# 13+ 时，将字段支持的 `[ObservableProperty]` 转换为 partial property。
...
详见 [NavigationAware](/zh-cn/generators/navigation-aware)。
...
详见 [DialogAware](/zh-cn/generators/dialog-aware)。
