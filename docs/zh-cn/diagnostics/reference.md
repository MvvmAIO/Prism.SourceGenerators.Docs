...
**代码修复：** **PSG0001–PSG0005** 均可在 IDE 中使用 **MakePartial**（**Ctrl+.** / **Alt+Enter**）。
...
详见 [DelegateCommand](../generators/delegate-command.md) 与 [AsyncDelegateCommand](../generators/async-delegate-command.md)。
...
详见 [容器注册](../generators/container-registration.md)。
...
`[NotifyDataErrorInfo]` 仅在类型继承自 `BindableValidator` 或使用了 `[BindableValidator]` 属性时才有效。否则生成的 setter 不会调用 `ValidateProperty`。详见 [BindableValidator](../generators/bindable-validator.md)。

## 下一步

- [源生成器总览](../generators/index.md)
